...
def default_position_id():
    ...
    o, _ = Position.objects.get_or_create(name="Без должности")
    return o.pk
    ...
    def clean(self):
        if self.dismissal_date and self.dismissal_date < self.hire_date:
            raise ValidationError(
                {"dismissal_date": "Дата увольнения не может быть раньше даты приема"}
            )

    def __str__(self):
        u = getattr(self.user, "get_username", lambda: str(self.user))()
        return f"{u} — {self.organization} ({self.position})"
